...
link_files_path = "E:\\TobTest\\linkfiles\\"
apk_dl_Path = "E:\\TobTest\\apk_dl\\"
log_Path = "E:\\TobTest\\logs\\"
...
class AppiumInit(object):
    ...
    def confirm_click(self, __apk_name, apkname, *dl_url):
        try:
            judge = Judgement()
            judge.judge_pic2(__apk_name, dl_url)
            time.sleep(15)
            driver.find_element_by_id(apkname + ":id/lebian_positiveButton").click()
            return True
...
class Emails(object):
    ...
    def send_email(self, sub, imgpath, *dl_url):
        ...
        smtp.sendmail(self.username, self.receiver, msg.as_string())
...
class Judgement(object):
    ...
    def judge_pic2(self, apkname, *dl_url):
        emails = Emails()
        sc = self.__get_picture_path(apkname)
        print("截图中……")
        ...
        if imf > 0.98 or imf2 > 0.98:
            ...
            if im > 0.98 or im2 > 0.98:
                b = "白屏时间过长或黑屏时间过长+" + sc
                print(b)
                emails.send_email(apkname + "白屏时间过长或黑屏时间过长", sc, dl_url)
            else:
                os.remove(sc)  # 没问题就删除图片
                return True
        elif ishan > 0.6:
            try:
                ...
                ss = "闪退了+" + sc
                print(ss)
                emails.send_email(apkname + "闪退了", sc, dl_url)
            return False
        else:
            os.remove(sc)
...
class MainTest(object):
    def __init__(self):
        ...
        while b < 15:
            ...
            while 1:
                emails.get_mails()
                m1 = fo.get_file_md5(link_files_path + "prevercode.txt")
                m2 = fo.get_file_md5(link_files_path + "lastfile.txt")
                if m1 == m2:
                    time.sleep(600)
                    break
                else:
    ...
    def check_test(self):
        ...
        if time2 < 100:
            ...
            for fup in failed_up_pkgname:
                ...
                if "Success" in oo:
                    print("安装成功")
                    self.hot_updata(apk_dl_Path + fup, fup)
            ...
            while 1:
                ...
                print(pt)
                if time3 >= 100 or pt % 100 <= 2:  # 如果时间大于1小时或者当前时间刚过整点，则重新拉取邮箱内容
                    emails.get_mails()
                    fo.confirm_md5()
                    self.check_test()
                    break
    ...
    def hot_updata(self, apkpath, apkname, *dl_url):
        ...
        try:
            ...
            return
        time.sleep(2)
        judgement.judge_pic2(apkname, dl_url)
        apk_name = driver.current_package
        # print(apk_name)
        while 1:
            if hot_flag == 1:
                while 1:
                    # judge = judgement.judge_pic2(apkname)
                    # if judge is False:
                    #     ww = ww + 1
                    #     break
                    # else:
                ...
                while 1:
                    click_cf = appium.confirm_click(apkname, apk_name, dl_url)  # 检测更新弹框并点击下载
...
if __name__ == "__main__":
    # MainTest().check_test()
    # Emails().get_mails()
    Emails().send_email('查收', "E:\\TobTest\\com.miquwan.sgzz-201808031506.png", "qqqq")
    # do_log().get_log()
    # do_log().stop_log()
    # MainTest()
    # MainTest().get_install('cdn.regengxin.com/origapk/otaapk/67756/11/gver5/lvl1/67756_11_5.1_com.yiwan.mssj.obb')
    # GetInfo().get_apk_name(r"E:\TobTest\apk_dl\67756_11_5.1_com.yiwan.mssj.apk")
