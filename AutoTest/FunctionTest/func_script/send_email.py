...
def get_conf():
    file = zipfile.ZipFile(os.path.join('E:\\', 'conf.zip'), 'r').read('conf.ini', pwd=bytes('654321', "utf8"))
    con = re.search(r"b'(.+)'", str(file)).group(1)
    conf = {}

    conf['username'] = con.split(' ')[0]
    conf['password'] = con.split(' ')[1]
    return conf
...
if __name__ == '__main__':
    email = EmailSending(username=input('邮箱：'), password=input('密码：'),
                         ...
                         image_path=BASE_PATH + '\\test_result1\\screenshot')
    email.screen_shot()
    email.create_email()
